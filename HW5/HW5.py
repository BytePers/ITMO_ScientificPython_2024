...
def sar_model_train(descriptors_train: pd.DataFrame, indices, best_params):
    '''
    ...
    a = Imputer(missing_values=np.nan, strategy='median')
    b = StandardScaler() 
    clf = ElasticNetCV(cv=best_params['anova__k'])
...
if __name__ == "__main__":
    ...
    best_params = model[0].best_params_


    # train the best estimator and predict values
    print("Training the model with the best parameters...")
    final_model = sar_model_train(train_descriptors, cols, best_params)
    for cpd in cpds:
